...
// HU8: actividades del formulario de caracterización (RN28) con cantidad absoluta y porcentaje (RN30).
export default function ActivitiesByType({ rows }: { rows: ReporteConteo[] }) {
  ...
  return <section className="admin-card report-avoid-break" aria-labelledby="activities-title">
    <div className="admin-section-heading"><h2 id="activities-title"><i className="bi bi-calendar2-week" aria-hidden="true" />Actividades por Tipo</h2><span className="admin-badge">Semilleros que las realizan</span></div>
    {!total ? <p role="status">No hay actividades registradas para los filtros aplicados.</p> : <div className="row g-4 align-items-center">
      <div className="col-lg-4"><svg viewBox="0 0 240 240" className="composition-donut" role="img" aria-label={`Actividades por tipo: ${total} registros`}>
        ...
        <text x="120" y="118" textAnchor="middle" className="campus-total">{numero(total)}</text><text x="120" y="142" textAnchor="middle" className="campus-caption">registros</text>
      </svg></div>
